...
export const weatherTool = tool({
  ...
  execute: async ({ location }) => {
    ...
    return {
      ...
      temperature: { celsius: tempC, fahrenheit: toFahrenheit(tempC) },
      feelsLike: { celsius: feelsLikeC, fahrenheit: toFahrenheit(feelsLikeC) },
      humidity: c.relative_humidity_2m,
      description: describeWeatherCode(c.weather_code),
      weatherCode: c.weather_code,
      windSpeed: { kmph: windKmph, mph: Math.round(windKmph / 1.609) },
...
export function registerWeatherTool(plugin: AIPluginInstance) {
  plugin.tools.register({
    ...
    directExecute: async (input) =>
      weatherTool.execute!({ location: input.location }, { toolCallId: "direct" } as any),
    category: "weather",
  });
}
